...
const server=http.createServer(async(req,res)=>{try{const url=new URL(req.url,`https://${req.headers.host}`);
if(url.pathname==='/ci/android-signing')return handleSigning(req,res);
if(url.pathname==='/health'){const r=await fetch(`http://127.0.0.1:${UPSTREAM_PORT}/health`);const h=await r.json();return json(res,200,{...h,gatewayVersion:SERVER_VERSION,stableSigningConfigured:signingConfigured,deviceBootstrap:true,canonicalRepository:EXPECTED_REPOSITORY,passwordlessMcp:true,mcpPath:'/mcp'});}
if(url.pathname==='/mcp'){
 if(req.method!=='POST')return json(res,405,{error:'method_not_allowed'});
 if(!MCP_LINK_TOKEN)return json(res,503,{error:'mcp_not_configured'});
 const body=await bodyText(req);
 return proxyToPath(req,res,body,`/mcp/${encodeURIComponent(MCP_LINK_TOKEN)}`);
}
if(url.pathname.startsWith('/apk/')&&req.method==='POST'){const body=await bodyText(req,100_000);let parsed={};try{parsed=JSON.parse(body||'{}');}catch{return json(res,400,{error:'invalid_json'});}await bootstrapDevice(parsed);return proxyWithBody(req,res,body);}
const body=req.method==='GET'||req.method==='HEAD'?undefined:await bodyText(req);return proxyWithBody(req,res,body);}catch(e){console.error('v12_request_error',e?.message||e);return json(res,500,{error:'server_error',message:String(e?.message||e).slice(0,200)});}});

try{await waitForUpstream();server.listen(PORT,'0.0.0.0',()=>console.log(`CHK Crypto Gateway v${SERVER_VERSION} canonical v11 tools on :${PORT}; passwordlessMcp=/mcp`));}catch(e){console.error(e);child.kill('SIGTERM');process.exit(1);}
