...
async function latestAndroidBybit(){if(!EDGE_URL||!SUPABASE_MCP_TOKEN)throw new Error('snapshot bridge not configured');const r=await fetch(edgeUrl('chk-crypto-sync'),{method:'POST',headers:{'content-type':'application/json','x-chk-token':SUPABASE_MCP_TOKEN,accept:'application/json','user-agent':'chk-crypto-v13'},body:JSON.stringify({action:'server_latest',exchange:'BYBIT'})});const text=await r.text();let root;try{root=JSON.parse(text||'{}');}catch{root={};}if(!r.ok)throw new Error(`snapshot ${r.status}: ${root?.error||text.slice(0,160)}`);return root?.record||null;}
...
async function handleOne(msg){if(msg?.method==='tools/list'){const out=await upstreamRpc(msg);const tools=Array.isArray(out?.result?.tools)?out.result.tools:[];if(!tools.some(t=>t?.name==='diagnose_bybit_identity'))tools.push({name:'diagnose_bybit_identity',title:'Diagnostic identité Bybit',description:'Compare sans exposer les clés l’empreinte Bybit Render à celle synchronisée par l’APK CHK Crypto.',inputSchema:{type:'object',properties:{},additionalProperties:false}});if(out?.result)out.result.tools=tools;return out;}
 ...
 if(msg?.method==='tools/call'){
  ...
  if(name==='get_bybit_portfolio_summary'||name==='list_bybit_assets'||name==='get_workspace_info'){
   ...
   if(name==='get_workspace_info'&&!isErr){const sc=upstream?.result?.structuredContent||{};sc.bybitIdentity=d;return result(msg.id,sc,`CHK Crypto Workspace v13 • dépôt canonique Chasmet/Binance-bybyt- • identité Bybit ${d?.identityMatch?'OK':'à corriger'}.`);}
   ...
   const sc=upstream?.result?.structuredContent||{};sc.bybitIdentity=d;sc.bybitFallback=portfolioFromSnapshot(rec);return result(msg.id,sc,'Workspace v13: Bybit Render non fiable, fallback Android disponible.');
  }
 }
