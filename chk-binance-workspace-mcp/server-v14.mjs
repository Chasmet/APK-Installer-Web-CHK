...
async function bridge(payload){const r=await fetch(bridgeUrl(),{method:'POST',headers:{'content-type':'application/json','x-chk-internal-token':CHK_INTERNAL_TOKEN,accept:'application/json','user-agent':'chk-crypto-workspace-v14.1'},body:JSON.stringify(payload)});const text=await r.text();let data;try{data=JSON.parse(text||'{}');}catch{data={raw:text};}if(!r.ok)throw new Error(`chk-mcp-bridge ${r.status}: ${data?.error||data?.message||text.slice(0,180)}`);return data;}
...
function parseCompatPayload(content){let p;try{p=JSON.parse(String(content||''));}catch{throw new Error('Pour kind ALERT/ALERT_UPDATE/ALERT_DELETE, content doit être un objet JSON valide.');}if(!p||typeof p!=='object'||Array.isArray(p))throw new Error('Payload alarme invalide');return p;}
...
async function handleOne(msg){
 if(msg?.method==='initialize'){const out=await upstreamRpc(msg);if(out?.result){out.result.serverInfo={name:'chk-crypto-workspace',version:SERVER_VERSION};out.result.instructions='CHK Crypto v14.1. Notes, ordres et annulations utilisent le pont authentifié. Compatibilité alarmes: si create_alert n’est pas visible à cause du cache ChatGPT, utiliser create_note avec kind=ALERT et content JSON {"symbol":"ADAUSDC","condition":"below","target_price":0.216,"label":"...","rationale":"...","one_shot":true}; kind=ALERT_UPDATE pour modifier avec id, kind=ALERT_DELETE pour supprimer avec id. list_notes retourne aussi alerts. Aucun ordre réel sans confirmation APK.';}return out;}
 ...
 if(name==='list_notes'){
  ...
  const d={...notes,alerts:Array.isArray(alerts?.alerts)?alerts.alerts:[],alarmCompatibility:true};
  return result(msg.id,d,`Bloc-notes CHK Crypto: ${Array.isArray(notes?.notes)?notes.notes.length:0} note(s) • ${d.alerts.length} alarme(s).`);
 }
 if(name==='create_note'){
