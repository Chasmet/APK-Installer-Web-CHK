...
async function bridge(payload){return internalPost(bridgeUrl(),payload,'chk-crypto-workspace-v14.4');}
async function chartBridge(payload){return internalPost(chartUrl(),payload,'chk-crypto-workspace-v14.4-analysis-compat');}
...
function appendCompatHints(out){
 ...
 out.result.structuredContent={...sc,analysisControlFallback:ANALYSIS_FALLBACK,delegatedAutoTrade:DELEGATED_AUTO_TRADE};
...
async function handleOne(msg){
 if(msg?.method==='initialize'){const out=await upstreamRpc(msg);if(out?.result){out.result.serverInfo={name:'chk-crypto-workspace',version:SERVER_VERSION};out.result.instructions='CHK Crypto v14.4. IMPORTANT cache: si les outils Analyse dédiés ne sont pas visibles, contrôle via create_note kind=ANALYSIS_CONTROL. AUTO-TRADE DÉLÉGUÉ: directMcpTrading=false ne signifie pas exécution automatique impossible. Sur CHK Crypto 0.9.7+, après autorisation utilisateur dans l APK (Auto-Trade actif + Auto-confirmer les propositions ChatGPT), create_trade_proposal peut créer une nouvelle proposition LIMIT que l APK exécute automatiquement sur Bybit EU Spot dans ses plafonds. Les propositions antérieures à l activation ne sont pas auto-exécutées.';}return out;}
 ...
 if(name==='list_notes'){
  ...
  const d={...notes,alerts:Array.isArray(alerts?.alerts)?alerts.alerts:[],alarmCompatibility:true,analysisControlCompatibility:true,analysisControlFallback:ANALYSIS_FALLBACK,delegatedAutoTrade:DELEGATED_AUTO_TRADE};
  return result(msg.id,d,`Bloc-notes CHK Crypto: ${Array.isArray(notes?.notes)?notes.notes.length:0} note(s) • ${d.alerts.length} alarme(s). IMPORTANT: si les outils Analyse sont cachés, utilise create_note kind=ANALYSIS_CONTROL. Auto-Trade délégué possible via create_trade_proposal si l autorisation correspondante est activée dans CHK Crypto 0.9.7+.`);
 }
 if(name==='create_note'){
...
const server=http.createServer(async(req,res)=>{try{const raw=req.method==='GET'||req.method==='HEAD'?undefined:await bodyText(req);const u=new URL(req.url,`https://${req.headers.host}`);if(u.pathname==='/mcp'&&req.method==='POST'){let parsed;try{parsed=JSON.parse(raw||'{}');}catch{return json(res,400,{jsonrpc:'2.0',id:null,error:{code:-32700,message:'Parse error'}});}const out=Array.isArray(parsed)?await Promise.all(parsed.map(handleOne)):await handleOne(parsed);return json(res,200,out,{'mcp-protocol-version':'2025-06-18'});}if(u.pathname==='/health'){let upstream={};try{const r=await fetch(`http://127.0.0.1:${UPSTREAM_PORT}/health`);upstream=await r.json();}catch{}return json(res,200,{...upstream,gatewayVersion:SERVER_VERSION,mcpBridge:true,alarmCompatibilityViaNotes:true,analysisControlViaNotes:true,analysisFallbackAdvertisedOnCachedReads:true,directMcpTrading:false,delegatedAutoTrade:true,autoTradeViaProposal:true,delegatedAutoTradeInfo:DELEGATED_AUTO_TRADE});}return proxy(req,res,raw);}catch(e){console.error('v14_request_error',e?.message||e);return json(res,500,{error:'server_error',message:String(e?.message||e).slice(0,240)});}});
try{await waitForUpstream();server.listen(PORT,'0.0.0.0',()=>console.log(`CHK Crypto Gateway v${SERVER_VERSION} cached-read Analyse + delegated Auto-Trade fallback on :${PORT}`));}catch(e){console.error(e);child.kill('SIGTERM');process.exit(1);}