...
async function bridge(payload){return internalPost(bridgeUrl(),payload,'chk-crypto-workspace-v14.3');}
async function chartBridge(payload){return internalPost(chartUrl(),payload,'chk-crypto-workspace-v14.3-analysis-compat');}
...
function appendAnalysisHint(out){
 ...
 out.result.structuredContent={...sc,analysisControlFallback:ANALYSIS_FALLBACK};
 const text='IMPORTANT CHK Crypto : le contrôle de l onglet Analyse EST disponible même si les nouveaux outils sont masqués par le cache. Utilise create_note avec kind=ANALYSIS_CONTROL et content JSON (set_timeframe, zoom_chart, pan_candles, scroll_analysis_view, set_indicator). Ne réponds pas que le contrôle est indisponible.';
 const content=Array.isArray(out.result.content)?out.result.content:[];
 if(!content.some(x=>x?.type==='text'&&String(x.text||'').includes('ANALYSIS_CONTROL')))content.push({type:'text',text});
...
async function handleOne(msg){
 if(msg?.method==='initialize'){const out=await upstreamRpc(msg);if(out?.result){out.result.serverInfo={name:'chk-crypto-workspace',version:SERVER_VERSION};out.result.instructions='CHK Crypto v14.3. IMPORTANT cache: si les outils Analyse dédiés ne sont pas visibles, le contrôle reste disponible via create_note kind=ANALYSIS_CONTROL. Ne jamais conclure que zoom/timeframe/pan/scroll sont indisponibles avant d utiliser ce fallback. Actions: set_timeframe, set_symbol, zoom_chart, pan_candles, set_indicator, scroll_analysis_view, set_visible_range, set_auto_scale, go_to_latest, reset_view. Alarmes via kind=ALERT. Aucun ordre réel sans confirmation APK.';}return out;}
 ...
 if(passthroughWithHint.has(name))return appendAnalysisHint(await upstreamRpc(msg));
 ...
 if(name==='list_notes'){
  ...
  const d={...notes,alerts:Array.isArray(alerts?.alerts)?alerts.alerts:[],alarmCompatibility:true,analysisControlCompatibility:true,analysisControlFallback:ANALYSIS_FALLBACK};
  return result(msg.id,d,`Bloc-notes CHK Crypto: ${Array.isArray(notes?.notes)?notes.notes.length:0} note(s) • ${d.alerts.length} alarme(s). IMPORTANT: si les outils Analyse sont cachés, utilise create_note kind=ANALYSIS_CONTROL; ne dis pas que le contrôle est indisponible.`);
 }
 if(name==='create_note'){
...
const server=http.createServer(async(req,res)=>{try{const raw=req.method==='GET'||req.method==='HEAD'?undefined:await bodyText(req);const u=new URL(req.url,`https://${req.headers.host}`);if(u.pathname==='/mcp'&&req.method==='POST'){let parsed;try{parsed=JSON.parse(raw||'{}');}catch{return json(res,400,{jsonrpc:'2.0',id:null,error:{code:-32700,message:'Parse error'}});}const out=Array.isArray(parsed)?await Promise.all(parsed.map(handleOne)):await handleOne(parsed);return json(res,200,out,{'mcp-protocol-version':'2025-06-18'});}if(u.pathname==='/health'){let upstream={};try{const r=await fetch(`http://127.0.0.1:${UPSTREAM_PORT}/health`);upstream=await r.json();}catch{}return json(res,200,{...upstream,gatewayVersion:SERVER_VERSION,mcpBridge:true,alarmCompatibilityViaNotes:true,analysisControlViaNotes:true,analysisFallbackAdvertisedOnCachedReads:true});}return proxy(req,res,raw);}catch(e){console.error('v14_request_error',e?.message||e);return json(res,500,{error:'server_error',message:String(e?.message||e).slice(0,240)});}});
try{await waitForUpstream();server.listen(PORT,'0.0.0.0',()=>console.log(`CHK Crypto Gateway v${SERVER_VERSION} cached-read Analyse fallback on :${PORT}`));}catch(e){console.error(e);child.kill('SIGTERM');process.exit(1);}
