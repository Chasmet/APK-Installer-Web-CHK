...
async function bridge(payload){const r=await fetch(bridgeUrl(),{method:'POST',headers:{'content-type':'application/json','x-chk-internal-token':CHK_INTERNAL_TOKEN,accept:'application/json','user-agent':'chk-crypto-workspace-v15'},body:JSON.stringify(payload)});const text=await r.text();let data;try{data=JSON.parse(text||'{}');}catch{data={raw:text};}if(!r.ok)throw new Error(`chk-mcp-bridge ${r.status}: ${data?.error||data?.message||text.slice(0,180)}`);return data;}
...
const alertTools=[
 ...
 {name:'list_alerts',title:'Lister les alarmes CHK Crypto',description:'Liste les alarmes de prix CHK Crypto, actives ou déclenchées.',inputSchema:{type:'object',properties:{limit:{type:'integer',minimum:1,maximum:200}},additionalProperties:false}},
 ...
 {name:'delete_alert',title:'Supprimer une alarme CHK Crypto',description:'Supprime réellement une alarme de l’onglet Alarmes CHK Crypto.',inputSchema:{type:'object',properties:{id:{type:'string'}},required:['id'],additionalProperties:false}},
];
...
async function handleOne(msg){
 if(msg?.method==='initialize'){
  ...
  if(out?.result){out.result.serverInfo={name:'chk-crypto-workspace',version:SERVER_VERSION};out.result.instructions='CHK Crypto v15. Le MCP gère portefeuille, marchés, notes, propositions d’ordres, annulations et alarmes. Les alarmes créées/modifiées/supprimées par MCP sont synchronisées dans l’onglet Alarmes de l’APK. Aucun ordre réel sans confirmation utilisateur dans l’APK.';}
  return out;
 }
