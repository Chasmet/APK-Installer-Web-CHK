...
function json(res, status, data) { const body=JSON.stringify(data); res.writeHead(status,{'content-type':'application/json; charset=utf-8','cache-control':'no-store, no-cache, must-revalidate',pragma:'no-cache',expires:'0','x-content-type-options':'nosniff','content-length':Buffer.byteLength(body)}); res.end(body); }
...
async function bootstrapDevice(body){
  ...
  const deviceId=String(body?.deviceId||'').trim(); const deviceSecret=String(body?.deviceSecret||'');
  if(!/^[a-f0-9-]{32,80}$/i.test(deviceId)||deviceSecret.length<32) throw new Error('invalid_device_credentials');
  const fingerprints={}; if(BINANCE_API_KEY) fingerprints.BINANCE=sha256(BINANCE_API_KEY); if(BYBIT_API_KEY) fingerprints.BYBIT=sha256(BYBIT_API_KEY);
  const r=await fetch(edgeUrl('chk-device-bootstrap'),{method:'POST',headers:{'content-type':'application/json','x-chk-token':SUPABASE_MCP_TOKEN,'user-agent':'chk-crypto-gateway-v9'},body:JSON.stringify({deviceId,deviceSecret,fingerprints})});
  if(!r.ok) throw new Error(`device_bootstrap_${r.status}`);
}
...
async function proxyWithBody(req,res,body){const upstream=new URL(req.url,`http://127.0.0.1:${UPSTREAM_PORT}`);const response=await fetch(upstream,{method:req.method,headers:{...(req.headers['content-type']?{'content-type':req.headers['content-type']}:{}),...(req.headers.accept?{accept:req.headers.accept}:{}),...(req.headers['mcp-protocol-version']?{'mcp-protocol-version':req.headers['mcp-protocol-version']}:{}),...(req.headers.authorization?{authorization:req.headers.authorization}:{})},body});const text=await response.text();res.writeHead(response.status,{'content-type':response.headers.get('content-type')||'application/json; charset=utf-8','cache-control':'no-store'});res.end(text);}
async function proxy(req,res){const body=req.method==='GET'||req.method==='HEAD'?undefined:await bodyText(req);return proxyWithBody(req,res,body);}

const server=http.createServer(async(req,res)=>{try{const url=new URL(req.url,`https://${req.headers.host}`);if(url.pathname==='/ci/android-signing')return await handleSigning(req,res);if(url.pathname==='/health'){const r=await fetch(`http://127.0.0.1:${UPSTREAM_PORT}/health`);const upstream=await r.json();return json(res,200,{...upstream,gatewayVersion:SERVER_VERSION,stableSigningConfigured:signingConfigured,deviceBootstrap:true,validationPrEnabled:Boolean(ALLOWED_PR_REF&&ALLOWED_HEAD_REF)});}
  if(url.pathname.startsWith('/apk/')&&req.method==='POST'){
    const body=await bodyText(req,100_000); let parsed={}; try{parsed=JSON.parse(body||'{}');}catch{return json(res,400,{error:'invalid_json'});}
    await bootstrapDevice(parsed);
    return await proxyWithBody(req,res,body);
  }
